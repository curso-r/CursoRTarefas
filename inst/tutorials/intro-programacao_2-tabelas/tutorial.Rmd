...
Use o data.frame `airquality` para responder às questões abaixo. __Vamos calcular o desvio padrão de Ozone sem usar a função `sd()`__. O resultado de um exercício é usado no próximo, então iremos guardar o resultado de cada etapa em um objeto.

a) Calcule a média da coluna `Ozone` (excluindo os NAs), e guarde o resultado em um objeto chamado `media_ozone`. 
...
```{r tabelas-desafio-b, exercise = TRUE, exercise.setup = "tabelas-desafio-a"}
# b) 
...
```{r tabelas-desafio-c, exercise = TRUE, exercise.setup = "tabelas-desafio-b"}
# c) 
...
```{r tabelas-desafio-d, exercise = TRUE, exercise.setup = "tabelas-desafio-c"}
# d) 
...
e) Calcule a raíz quadrada, e guarde o resultado em um objeto chamado `desvio_padrao_ozone_calculado`.

```{r tabelas-desafio-e, exercise = TRUE, exercise.setup = "tabelas-desafio-d"}
# e)
...
```{r tabelas-desafio-f, exercise = TRUE, exercise.setup = "tabelas-desafio-e"}
# f)
sd_ozone_funcao <- #___  
desvio_padrao_ozone_calculado #___ sd_ozone_funcao
```
...
grade_this({
  if (identical(.result, .solution)) {
    pass("Correto! Os resultados são diferentes! Isso porque ...")
  }
  fail()
})
```

<!-- para resolver no desafio: -->
<!-- - aqui tá aparecendo uma mensagem de erro esquisita nos exercicios:  -->
<!-- <text>:3:0: unexpected end of input 1: # a) 2: media_ozone <- #___ ^ -->
<!-- - as mensagens de `pass` nao estao aparecendo corretamente para os exercicios b até f -->
